...
if __name__ == '__main__':
    ...
    ************************************
    ************************************
        if PARAMS  条件编译 指令是否有参数 #If params has any instructions
            0  指令不含参数
            1   指令含参数
        
        #following instructions include:    
        包括以下指令的例子：
            EnableRobot
            DisableRobot
            DO
    ...
    ************************************
    ************************************
    Function to enable the robot: EnableRobot
    """
    if PARAMS:
      dashboard.EnableRobot()    #无参数 Parameters Indicated
    else:
       load=0.1
       centerX=0.1
       centerY=0.1
       centerZ=0.1
       dashboard.EnableRobot(load)    #一个参数 Parameters=0 Default Load
    ...
    ************************************
     ...
     * 功能：设置数字输出端口状态（队列指令）
    """
    index=1 # digital output index, range: 1 to 16 or 100 to 1000.
    status=1 #status of the digital output port. 1: High level; 0: Low level
    ...
    """
    # Set the JointJ Accelerations; joint acceleration rate, range: 1~100
    acceleration = 5
